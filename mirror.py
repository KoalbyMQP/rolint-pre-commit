...
def get_all_versions() -> list[Version]:
    response = urllib3.request("GET", "https://pypi.org/pypi/rolint/json")
...
def get_current_version(pyproject: dict) -> Version:
    ...
    requirement = next((r for r in requirements if r.name == "rolint"), None)
    assert requirement is not None, "pyproject.toml does not have rolint requirement"

    specifiers = list(requirement.specifier)
    assert (
        len(specifiers) == 1 and specifiers[0].operator == "=="
    ), f"rolint's specifier should be exact matching, but `{requirement}`"
...
def process_version(version: Version) -> typing.Sequence[str]:
    def replace_pyproject_toml(content: str) -> str:
        return re.sub(r'"rolint==.*"', f'"rolint=={version}"', content)

    def replace_readme_md(content: str) -> str:
        # Update version references in README if you have them
    ...
    # Add README.md if it exists and has version references
    readme_path = Path("README.md")
    if readme_path.exists():
        paths["README.md"] = replace_readme_md
